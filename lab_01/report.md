...
$\psi_2(\eta_2) = \pi -\arcsin{\eta_2} \quad \psi_2'(\eta_1) = -\frac{1}{\sqrt{1 - \eta_2^2}}$
...
2. $\eta_1, \eta_2$ - **зависимы**, так как есть тождество: $\cos{\xi}^2 + \sin{\xi}^2 = 1$


### 5. Найти плотность распределения суммы двух независимых случайных величин $\xi$ и $\eta$. 
...
1. $\xi \in [0; 1], t \in [0; \xi]$ 
   ...
   $\int_{0}^{\xi}2e^{-2t}dt = 1 - e^{-2\xi}$

2. $\xi > 1, t \in [\xi - 1; \xi]$
    
    $\int_{\xi - 1}^{\xi}2e^{-2t}dt = e^{2(1 - \xi)} - e^{-2\xi}$
